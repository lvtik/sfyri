...
static char effective_image_path[MAX_IMAGE_PATH] = {0};
...
static void trim_path(char *path) {
    ...
    while (len > 0 && isspace((unsigned char)path[len - 1])) path[--len] = '\0';

    size_t start = 0;
    while (path[start] != '\0' && isspace((unsigned char)path[start])) start++;
    if (start > 0) memmove(path, path + start, len - start + 1);
}

static void expand_home(char *path, size_t size) {
    if (path[0] != '~') return;
    if (path[1] != '\0' && path[1] != '/' && path[1] != '\\') return;
    ...
    char rest[MAX_IMAGE_PATH];
    snprintf(rest, sizeof(rest), "%s", path + 1);
    snprintf(path, size, "%s%s", home, rest);
}

/* Manually typed paths often carry stray whitespace or a shell-style "~"
   that the filesystem APIs never expand on their own; without this, a
   perfectly valid typed path silently reads back as "no image loaded". */
static void normalize_manual_path(const char *raw, char *out, size_t out_size) {
    snprintf(out, out_size, "%s", raw);
    trim_path(out);
    expand_home(out, out_size);
}
...
static void open_file_picker(void) {
    ...
    if (effective_image_path[0] != '\0') {
        if (DirectoryExists(effective_image_path)) {
            directory = effective_image_path;
        } else {
            directory = GetDirectoryPath(effective_image_path);
        }
    }
...
static void refresh_disks(void) {
    ...
    if (selected_disk >= 0 && (size_t)selected_disk < disks.count) {
        snprintf(previous_path, sizeof(previous_path), "%s", disks.items[selected_disk].path);
    }
    ...
    /* Keep the same physical disk selected across a refresh instead of
       silently snapping back to index 0 -- with a data-destroying "Burn"
       button on the other end of this, that would be a nasty surprise. */
    selected_disk = -1;
    if (previous_path[0] != '\0') {
        for (size_t i = 0; i < disks.count; i++) {
            if (strcmp(disks.items[i].path, previous_path) == 0) {
                selected_disk = (int)i;
                break;
            }
        }
    }
    if (selected_disk < 0) selected_disk = (disks.count > 0) ? 0 : -1;
    ...
    for (size_t i = 0; i < disks.count; i++) {
        char label[1400];
        const char *media = (disks.items[i].label[0] != '\0') ? disks.items[i].label : "Unknown disk";

        if (disks.items[i].is_internal) {
            const char *tagged = GuiIconText(ICON_WARNING, TextFormat("INTERNAL  %s", media));
            snprintf(label, sizeof(label), "%s    %s    %s", tagged, disks.items[i].name, disks.items[i].path);
        } else {
            snprintf(label, sizeof(label), "%s    %s    %s", media, disks.items[i].name, disks.items[i].path);
        }
...
static bool inputs_ready(long long image_size) {
    return image_size >= 0 &&
        selected_disk >= 0 &&
        (size_t)selected_disk < disks.count;
}

static bool selected_inputs(BurnImage *image, const char **device, long long image_size) {
    if (effective_image_path[0] == '\0') {
    ...
    snprintf(image->path, sizeof(image->path), "%s", effective_image_path);
...
void main_ui_loop() {
    while (!WindowShouldClose() || burning) {
        BeginDrawing();
        ClearBackground(current_theme.background);

        bool modal_open = confirm_burn || show_file_picker || burning;
        ...
        Rectangle image_path_box = {48, 128, 548, 32};
        if (image_path_edit) {
            DrawRectangleLinesEx((Rectangle){image_path_box.x - 2, image_path_box.y - 2,
                image_path_box.width + 4, image_path_box.height + 4}, 2, current_theme.pressed);
        }
        if (GuiTextBox(image_path_box, image_path, MAX_IMAGE_PATH, image_path_edit)) {
            if (image_path_edit) {
                char normalized[MAX_IMAGE_PATH];
                normalize_manual_path(image_path, normalized, sizeof(normalized));
                snprintf(image_path, sizeof(image_path), "%s", normalized);
            }
        ...
        normalize_manual_path(image_path, effective_image_path, sizeof(effective_image_path));
        long long image_size = (effective_image_path[0] == '\0') ? -1 : get_image_size(effective_image_path);
        bool valid_inputs = inputs_ready(image_size);
        if (image_size >= 0) {
            GuiLabel((Rectangle){716, 132, 136, 24}, format_size(image_size));
        } else if (effective_image_path[0] != '\0') {
            GuiSetStyle(DEFAULT, TEXT_COLOR_NORMAL, ColorToInt(current_theme.error));
            GuiLabel((Rectangle){716, 132, 136, 24}, "File not found");
            GuiSetStyle(DEFAULT, TEXT_COLOR_NORMAL, ColorToInt(current_theme.text));
        } else {
        ...
        if (selected_disk >= 0 && (size_t)selected_disk < disks.count) {
            bool selected_is_internal = disks.items[selected_disk].is_internal;
            if (selected_is_internal) GuiSetStyle(DEFAULT, TEXT_COLOR_NORMAL, ColorToInt(current_theme.error));
            GuiLabel((Rectangle){48, 428, 804, 24}, TextFormat("Selected: %s%s",
                disks.items[selected_disk].path,
                selected_is_internal ? "  (INTERNAL DISK - are you sure?)" : ""));
            if (selected_is_internal) GuiSetStyle(DEFAULT, TEXT_COLOR_NORMAL, ColorToInt(current_theme.text));
        } else {
        ...
        if (confirm_burn) {
            bool confirm_is_internal = (selected_disk >= 0 && (size_t)selected_disk < disks.count)
                ? disks.items[selected_disk].is_internal : false;
            const char *disk_path = (selected_disk >= 0 && (size_t)selected_disk < disks.count)
                ? disks.items[selected_disk].path : "?";
            const char *message = confirm_is_internal
                ? TextFormat(
                    "Write \"%s\" (%s)\nto %s\n\n"
                    "WARNING: this disk is marked INTERNAL.\nThis may be your system disk. This cannot be undone.",
                    GetFileName(effective_image_path), format_size(image_size), disk_path)
                : TextFormat(
                    "Write \"%s\" (%s)\nto %s\n\nThis cannot be undone.",
                    GetFileName(effective_image_path), format_size(image_size), disk_path);

            int result = GuiMessageBox(
                (Rectangle){225, 150, 450, 220},
                confirm_is_internal ? "Confirm burn - INTERNAL DISK" : "Confirm burn",
                message,
                "Cancel;Burn");
            ...
            if (result == 1) {
                ...
                confirm_burn = false;
                if (selected_inputs(&image, &device, image_size) && burn_start(image, device)) {
                    burning = true;
                    set_status(false, "Burning...");
                } else {
        ...
        if (burning) {
            long long burn_written = 0, burn_total = 0;
            BurnState state = burn_get_progress(&burn_written, &burn_total);

            if (state == BURN_RUNNING) {
                DrawRectangle(0, 0, GetScreenWidth(), GetScreenHeight(), current_theme.overlay);

                Rectangle dialog = {250, 210, 400, 130};
                GuiPanel(dialog, "Burning");
                GuiLabel((Rectangle){dialog.x + 16, dialog.y + 36, dialog.width - 32, 24},
                    TextFormat("%s / %s", format_size(burn_written), format_size(burn_total)));

                float fraction = (burn_total > 0) ? (float)((double)burn_written / (double)burn_total) : 0.0f;
                if (fraction > 1.0f) fraction = 1.0f;
                GuiProgressBar((Rectangle){dialog.x + 16, dialog.y + 68, dialog.width - 32, 20},
                    NULL, TextFormat("%.0f%%", fraction * 100.0f), &fraction, 0.0f, 1.0f);

                GuiLabel((Rectangle){dialog.x + 16, dialog.y + 98, dialog.width - 32, 20},
                    "Do not unplug the disk or close sfyri.");
            } else {
                bool success = (state == BURN_DONE);
                burn_reset();
                burning = false;
                if (success) {
